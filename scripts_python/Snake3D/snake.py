...
from serial import *  # Permet de communiquer avec le port série
from collections import (
    deque,
)  # Permet de faire des opérations avancées sur les listes (rotate)
...
def ActualiserCube():
    ...
    if perdu != 1 and (
        ...
        or snake[0][1] < 0
        or snake[0][2] < 0
        or snake[0][0] > dimension - 1
        or snake[0][1] > dimension - 1
        or snake[0][2] > dimension - 1
    ):
    ...
    for i in range(4, len(snake)):
        if (
            snake[0][0] == snake[i][0]
            and snake[0][1] == snake[i][1]
            and snake[0][2] == snake[i][2]
        ):
            fail_sound.play()
            perdu = 1
            music.stop()
    ...
    if (
        snake[0][0] == champi[0]
        and snake[0][1] == champi[1]
        and snake[0][2] == champi[2]
    ):
    ...
    for i in range(dimension * dimension):
        led_cube.matrice_leds.append([0] * dimension)
    ...
    for i in range(0, len(snake)):
        ...
        if i == 0:
            led_cube.matrice_leds[ligne][snake[i][0]] = 3  # La tête en violet
        # Sinon, et si on est vivant, le corps en bleu
        elif perdu != 1:
            led_cube.matrice_leds[ligne][snake[i][0]] = 2  # le corps en bleu
        # Enfin, si on est mort, le corps en rouge
        else:
            led_cube.matrice_leds[ligne][snake[i][0]] = 1  # le corps en rouge
    ...
    led_cube.matrice_leds[ligne][champi[0]] = 1  # le champi en rouge
...
def Nom_Joueur():
    ...
    Label(Name_Screen, text="Nom du joueur pour les scores :").grid()
    global namefield
...
def run():
    led_cube = LedCube(dimension, dimension, dimension)
