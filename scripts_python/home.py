...
def Open_display_commands():
    ...
    import DisplayCommands.display_commands

    DisplayCommands.display_commands.run()


def Open_snake():
    Mafenetre.destroy()

    import Snake3D.snake
...
SelectHint = Label(
    Mafenetre, text="Sélectionner le programme à exécuter :", anchor=NW, justify=LEFT
)
...
Button(Mafenetre, text="Interface de commande", command=Open_display_commands).grid(
    row=1, column=0, padx=5, pady=5
)
# Snake3D
Button(Mafenetre, text="Snake 3D", command=Open_snake).grid(
    row=1, column=1, padx=5, pady=5
)
# Space Defender
Button(Mafenetre, text="Space Defender", command=Open_space_defender).grid(
