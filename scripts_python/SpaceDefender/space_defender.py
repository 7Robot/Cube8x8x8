...
class Partie:
    ...
    def Nom_Joueur(self):
        def Joueur(event):
            ...
            PlayerLevel.config(
                text="Joueur : {0}\nDifficulté : {1}".format(
                    self.nom_joueur, self.difficulty
                )
            )
            woosh.play()
            Name_Screen.destroy()
        ...
        Label(Name_Screen, text="Nom du joueur :").grid(row=0, column=0, padx=6)
        namefield = Entry(
            Name_Screen, width=12, textvariable="{0}".format(self.nom_joueur)
        )
...
def Touche(event):
    ...
    if NouvellePartie.pause == 0:
        ...
        if (
            (event.keysym == "Up" and NouveauVaisseau.position[0] > 0)
            or (event.keysym == "Down" and NouveauVaisseau.position[0] < 7)
            or (event.keysym == "Left" and NouveauVaisseau.position[1] > 0)
            or (event.keysym == "Right" and NouveauVaisseau.position[1] < 7)
        ):
...
def Envoyer():
    ...
    try:
        ...
        for k in range(dimension):
            ...
            for i in range(dimension):
                dev.write(octets_bleus[k][i].to_bytes())
                dev.write(octets_rouges[k][i].to_bytes())
    except Exception:
